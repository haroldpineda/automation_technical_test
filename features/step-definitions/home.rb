...
find_element(id: "tool_button").click
end

And ("I want to define the tip percentage {string}") do |percentage|
find_element(id: "field_percentage").send_keys(percentage)
end

And ("I press the save setting button")do
find_element(id: "save_settings").click
end

And ("I valid the percentage {string} defined in the tip amount field") do |percentage|
expect(percentage).to contain_exactly((find_element(id: "textbox_percentaje").text)
end
